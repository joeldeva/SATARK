...
async def generate_seed_survey(request: Dict[str, Any], user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        fallback_generator = SurveyGenerator(_generator.parser, _generator.rag, _generator.rules, llm_planner=None)
        generated = fallback_generator.generate(prompt, user_id)
    ...
    requested_languages = _normalize_language_codes([language_code]) if language_code else None
    return {
        "survey": _generated_to_designer_survey(generated, requested_languages),
        "generated": generated,
...
async def generate_survey(request: Dict[str, Any], user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        logger.error("Generation failed; retrying with bounded fallback: %s", exc, exc_info=True)
        from services.survey_generator import SurveyGenerator

        fallback_generator = SurveyGenerator(_generator.parser, _generator.rag, _generator.rules, llm_planner=None)
        survey = fallback_generator.generate(prompt, user_id)
        elapsed = round(time.time() - start, 3)
        _log_generation_only(survey, prompt, user_id, elapsed)
        return {
            "success": True,
            ...
            "is_verdict": False,
            "needs_review": True,
            "note": "Draft generated by bounded SATARK assist and trusted rules - review before publishing",
        }
...
def _generated_to_designer_survey(generated: dict[str, Any], language_override: list[str] | None = None) -> dict[str, Any]:
    nodes = []
    languages = _normalize_language_codes(language_override or generated.get("languages") or ["en"])
    if language_override is None and len(languages) > 2:
        languages = ["en", next((language for language in languages if language != "en"), "en")]
